...
def get_dataset(name, **kwargs):
    if name == 'masked_bag_waterbirds':
        return MaskedBagWaterbirds(
            basedir=kwargs.pop("data_dir"),
            split=kwargs.pop("split"),
            transform=kwargs.pop("transform", None),
            bag_size=kwargs.get("bag_size", 5),
            masking_strategy=kwargs.get("masking_strategy", "mean_fill_dilation"),
            max_distance=kwargs.get("max_distance", 30.0),
            alpha=kwargs.get("alpha", 0.1),
            crop_scale=kwargs.get("crop_scale", (0.85, 1.0)),
            gradual_shrink=kwargs.get("gradual_shrink", False),
    ...
    elif name == "SpuriousDataset":
        # Normalize the keyword to 'basedir'
        if "data_dir" in kwargs:
            kwargs["basedir"] = kwargs.pop("data_dir")
        return SpuriousDataset(**kwargs)
    else:
